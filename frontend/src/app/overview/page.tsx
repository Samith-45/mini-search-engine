...
import { fetchLatestExperiment, fetchClusterTopology, fetchTelemetry } from '@/lib/api';
import { ExperimentRecord, ClusterTopology, HealthTelemetry } from '@/lib/types';
...
export default function RecruiterOverviewPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [exp, topo, telem] = await Promise.all([
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      {/* Verified Empirical Metrics at a Glance (From Latest Database Experiment) */}
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs font-mono text-slate-400">
          <span className="flex items-center gap-1.5 font-bold uppercase tracking-wider text-slate-300">
            <Activity className="w-4 h-4 text-sky-400" />
            Verified Experiment Baseline Metrics
          </span>
          {latestExp ? (
            <span className="text-emerald-400">
              ● Latest Experiment: {latestExp.experimentName} (Commit: {latestExp.gitCommit})
            </span>
          ) : (
            <span className="text-amber-400">
              ● No experiment recorded in database yet
            </span>
          )}
        </div>
        ...
        {latestExp ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="glass-panel p-5 rounded-xl border border-slate-800 bg-slate-900/40">
              <div className="text-xs text-slate-400 flex items-center gap-1.5"><Gauge className="w-3.5 h-3.5 text-sky-400" /> Measured Throughput</div>
              <div className="text-2xl font-extrabold font-mono text-sky-400 mt-1">{latestExp.queriesPerSec.toLocaleString()} QPS</div>
              <div className="text-[10px] text-slate-400 mt-0.5">{latestExp.concurrencyLevel} Concurrent Threads</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-xl border border-slate-800 bg-slate-900/40">
              ...
              <div className="text-2xl font-extrabold font-mono text-emerald-400 mt-1">{latestExp.p95LatencyMs} ms</div>
              <div className="text-[10px] text-slate-400 mt-0.5">{latestExp.documentCount} Docs ({latestExp.shardCount} Shards)</div>
            </div>

            <div className="glass-panel p-5 rounded-xl border border-slate-800 bg-slate-900/40">
              <div className="text-xs text-slate-400 flex items-center gap-1.5"><Database className="w-3.5 h-3.5 text-purple-400" /> Memory Footprint</div>
              <div className="text-2xl font-extrabold font-mono text-purple-400 mt-1">{latestExp.memoryUsedMb} MB</div>
              <div className="text-[10px] text-slate-400 mt-0.5">Heap Utilization</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-xl border border-slate-800 bg-slate-900/40">
              ...
              <div className="text-2xl font-extrabold font-mono text-amber-400 mt-1">
                {topology ? `${topology.primaryShardCount} Shards + ${topology.replicaShardCount} Rep` : '3 Shards + 3 Rep'}
              </div>
              <div className="text-[10px] text-slate-400 mt-0.5">Automated replica failover</div>
            </div>
          </div>
        ) : (
          <div className="p-6 rounded-2xl border border-dashed border-slate-800 bg-slate-900/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="text-sm font-bold text-slate-200 flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-amber-400" />
                No verified benchmark run recorded in backend database yet.
              </div>
              <p className="text-xs text-slate-400">
                SearchForge requires all displayed metrics to be generated from genuine execution. 
                Trigger a benchmark in the Benchmark Lab to record verified baseline measurements.
              </p>
            </div>
            <Link
              href="/engineering"
              className="px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs font-mono flex items-center gap-2 transition-all whitespace-nowrap shadow-lg shadow-sky-600/20"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-3">
            ...
            <ul className="space-y-1.5 text-slate-300">
              ...
              <li className="flex items-start gap-2">
                <span className="text-emerald-400 font-bold">✓</span>
                <span><strong>Empirical Load Runner:</strong> In-memory load generator executing concurrent query distributions with percentile latencies.</span>
              </li>
            </ul>
          </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-3">
            ...
            <ul className="space-y-1.5 text-slate-300">
              ...
              <li className="flex items-start gap-2">
                <span className="text-rose-400 font-bold">✓</span>
                <span><strong>Fault Injection Rig:</strong> Controlled node termination and latency injection with real pre-, degraded-, and post-recovery timing measurements.</span>
              </li>
            </ul>
          </div>
